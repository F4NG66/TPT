...
def main():
    ...
    seqs = load_fasta("../../data/gmsc.fasta")
    tokenizer = build_tokenizer()

    dataset = ProteinDataset(sequences=seqs, tokenizer=tokenizer, mode="pretrain")
    loader = DataLoader(dataset, batch_size=50, shuffle=True, num_workers=8)
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(state["model"], "best_pretrain.pt")
