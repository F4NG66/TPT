# 维度实验
# (4)Importantly, the embedding dimension size also must be explored by increasing and decreasing its size. Further, elaborate how look-up tale is learnt.:
#320 480 800
"""
###############################################################
#                        AMP Task
...
for  datasetusing in  DataSetList:
    ...
    #tiny_model.load_state_dict(torch.load("./Ablation/Hidden_dim/320/best_pretrain_1M.pt")) 
    #tiny_model.load_state_dict(torch.load("./Ablation/Hidden_dim/480/best_pretrain_1M.pt")) 
    #tiny_model.load_state_dict(torch.load("./Ablation/Hidden_dim/800/best_pretrain_1M.pt"))        
    
        
    tiny_model_320.load_state_dict(torch.load(f"./Ablation/Hidden_dim/320/best_pretrain_10M.pt")) 
    tiny_model_480.load_state_dict(torch.load(f"./Ablation/Hidden_dim/480/best_pretrain_10M.pt")) 
    tiny_model_640.load_state_dict(torch.load(f"./Ablation/Hidden_dim/640/best_pretrain_10M.pt")) 
    tiny_model_800.load_state_dict(torch.load(f"./Ablation/Hidden_dim/800/best_pretrain_10M.pt")) 
