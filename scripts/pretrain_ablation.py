...
from Ablation.tpt import TinyProteinTransformer # 对比学习在此 消融 消除loss
    ...
    for xr in  xiaorong:
     # 256 512 1024   之前是640
        ...
        if xr == "TPT_Contrast_Weight":
             model = TinyProteinTransformer(vocab_size=len(tokenizer)) #替换损失
        if xr == "TPT_Without_Attenpool_weight":
             model = TinyProteinTransformerWithoutAttentionPooling(vocab_size=len(tokenizer)) #正常
        if xr == "TPT_Without_CNN_Weight":
             model = TinyProteinTransformerWithoutCNN(vocab_size=len(tokenizer)) #正常
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            with open(f"loss_log{xr}_10M.json", "w") as f:
                json.dump(log_dict, f)

            save_loss_plot(log_dict,fname=f"./VisulPlot/{xr}_pretrain_loss_10M.png")
            ...
            torch.save(state, f"./Ablation/{xr}/checkpoint_10M.pt")
            print("Saved checkpoint.pt")

            if avg_loss < best_loss:
                best_loss = avg_loss
                torch.save(state["model"], f"./Ablation/{xr}/best_pretrain_10M.pt")
