# (5)The manuscript places strong emphasis on t-SNE visualizations to support representation quality. These results should be interpreted more cautiously, as such visualizations are qualitative and do not constitute rigorous evidence without additional quantitative analysis. :
# TSNE审稿人说你描述的太过了 改改说法柔和点就行了吧
...
for datasetusing in DataSetList:
    ...
    if datasetusing == "CRISP":
        df = pd.read_csv("./sequences_length_le100_CRISPR.csv")
    ...
    if datasetusing == "MOES":
        df = pd.read_csv("./short_sequences_MOES.csv")
    ...
    tiny_model.load_state_dict(torch.load("./Ablation/TPT_Weight/best_pretrain_10M.pt"),strict=False)
