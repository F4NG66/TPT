...
def main():
    ...
    for hidden_dim in  hidden_dim_list:
     # 256 512 1024   之前是640
        ...
        model = TinyProteinTransformer(vocab_size=len(tokenizer),hidden_dim =hidden_dim)
        # model = TinyProteinTransformer(vocab_size=len(tokenizer),hidden_dim =hidden_dim_list[1])
        # model = TinyProteinTransformer(vocab_size=len(tokenizer),hidden_dim =hidden_dim_list[2])
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            with open(f"./log/{hidden_dim}loss_log.json", "w") as f:
                json.dump(log_dict, f)

            save_loss_plot(log_dict,fname=f"./VisulPlot/{hidden_dim}_pretrain_loss_10M.png")
            ...
            torch.save(state, f"./Ablation/Hidden_dim/{hidden_dim}/checkpoint_10M.pt")
            print("Saved checkpoint.pt")

            if avg_loss < best_loss:
                best_loss = avg_loss
                torch.save(state["model"], f"./Ablation/Hidden_dim/{hidden_dim}/best_pretrain_10M.pt")
